...
import { wantsLightweightBackground } from '../../platform';

const DynamicBackground = React.memo(() => {
  const isLightweight = wantsLightweightBackground();

  return (
    <div className="app-background fixed inset-0 pointer-events-none -z-10 overflow-hidden bg-[#e5ecf6]">
      {isLightweight ? (
        <>
          <div className="app-background-orb absolute top-[-10%] left-[-10%] w-[70%] h-[70%] rounded-full bg-blue-300/25 blur-[60px]" />
          <div className="app-background-orb absolute bottom-[-10%] right-[-10%] w-[60%] h-[60%] rounded-full bg-rose-300/25 blur-[60px]" />
        </>
      ) : (
        <>
          <motion.div
